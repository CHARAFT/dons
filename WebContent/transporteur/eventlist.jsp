...
<div class="table-data">
				<div class="order">
					<div class="head">
						<h3>Recent Events</h3>
						<i class='bx bx-plus'><a href="#" class="document-details-button" data-toggle="modal" data-target="#add-event-modal">Ajouter Event</a></i>
			...
			<c:forEach items="${events}" var="event">
                <tr>
					...
					<td> ${event.debut} </td>
                    <td>${event.fin}</td>
					<td>${event.objectif}</td>                    
					<td>${event.lieu}</td>
					<td>
                <!-- Ajoutez des liens ou des boutons pour les actions -->
               <a href="#" class="document-details-button" data-toggle="modal" data-target="#document-modal-${event.id}"><i class="fa-solid fa-pen-to-square"></i></a> 
	                <a href="/dons/EventServlet?id=${event.id}&action=supprimer" onclick="return confirm('Voulez-vous vraiment supprimer cet event ?')"><i class="fa-solid fa-trash"></i></a>
                ...
                <div class="modal fade" id="document-modal-${event.id}" tabindex="-1" role="dialog" aria-labelledby="document-modal-label" aria-hidden="true">
						...
						<div class="modal fade" id="add-event-modal1" tabindex="-1" role="dialog" aria-labelledby="add-event-modal-label" aria-hidden="true">
						    <div class="modal-dialog modal-dialog-centered" role="document">
						        <div class="modal-content">
						            <div class="modal-header">
	...
	<div class="row row-cols-3 g-3">
			...
			<c:set var="imagePath" value="${event.image}" />
  			...
  			<div class="col">
				<div class="card" >
				  <img class="card-img-top" src="${pageContext.request.contextPath}${imagePath}" alt="Card image cap">
				  <div class="card-body">
				    <h5 class="card-title">${event.nom}</h5>
				    <p class="card-text"> ${event.debut}</p>
				        <p class="card-text"> ${event.fin}</p>
